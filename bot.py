...
class EURUSDTradingBot:
    ...
    def close_position(self, position_id=None):
        ...
        try:
            # Get open positions to verify they exist
            positions = self.oanda_client.get_open_positions()
            eur_usd_positions = [p for p in positions if p.get("instrument") == "EUR_USD"]
            
            logger.info(f"Found {len(eur_usd_positions)} EUR/USD positions to close")
            for pos in eur_usd_positions:
                logger.info(f"Position details: {pos}")
            
            if not eur_usd_positions:
                logger.warning("No open EUR/USD positions found to close")
                self.last_error = "No open EUR/USD positions found to close"
                return False
            ...
            if result.get("success", False):
                ...
                error_msg = f"Failed to close position: {result.get('error', 'Unknown error')}"
    ...
    def execute_decision(self, decision, price_data):
        ...
        try:
            ...
            if action == "WAIT":
                ...
                if result and hasattr(self.memory, 'record_trade'):
                    try:
                ...
                if result and hasattr(self.memory, 'update_last_trade'):
                    try:
                        self.memory.update_last_trade(is_closed=True)
                    except Exception as e:
                        # Don't fail the whole operation if recording fails
                        logger.warning(f"Failed to update trade record: {e}")
