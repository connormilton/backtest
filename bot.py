...
# Hard-coded parameters with updated risk profile
MAX_DAILY_DRAWDOWN = 0.10  # 10% max daily drawdown
MAX_ACCOUNT_RISK = 0.02  # 2% max total account risk (reduced from 10%)
INITIAL_SAFETY_LEVEL = 0.01  # Starting at 1% risk per trade (max allowed)
...
class EURUSDTradingBot:
    ...
    def calculate_current_exposure(self):
        ...
        try:
            ...
            for position in eur_usd_positions:
                ...
                price = float(position.get('averagePrice', 0))
                position_value = units * price / 10000  # Convert to standard lots
                total_exposure_value += position_value
            
            # Calculate exposure as percentage of account balance
            exposure_percent = (total_exposure_value / balance) * 100
            return exposure_percent
        except Exception as e:
            logger.error(f"Error calculating current exposure: {e}")
            # Return 0 if calculation fails to allow at least one trade
            return 0
    ...
    def execute_decision(self, decision, price_data):
        ...
        if action == "OPEN":
            ...
            # Calculate total exposure risk
            current_exposure = self.calculate_current_exposure()
            
            # Check if new trade would exceed maximum account risk of 2%
            if current_exposure + risk_percent > MAX_ACCOUNT_RISK * 100:
                logger.warning(f"Trade would exceed maximum account risk: current exposure {current_exposure:.2f}%, " + 
                             f"new trade risk {risk_percent:.2f}%, max allowed {MAX_ACCOUNT_RISK*100:.2f}%")
                return False
            ...
            validation_result = self.brain.validate_strategy(
                strategy_details, 
                price_data, 
                risk_level=0.01,  # Enforce 1% risk per trade 
