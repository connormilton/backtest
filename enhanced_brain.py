...
class EnhancedTradingBrain:
    ...
    def __init__(self, memory, backtester=None):
    ...
    def _run_backtest_validation(self, trade_details, price_data, lookback_period=30, 
        ...
        try:
            ...
            if self.backtester is None:
                logger.warning("No backtester available for validation")
                return {
                    "valid": True,  # Default to valid if we can't check
                    "reason": "No backtester available for validation",
                    "metrics": {}
                }
